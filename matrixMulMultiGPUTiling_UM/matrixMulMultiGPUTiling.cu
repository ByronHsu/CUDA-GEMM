...
int main(int argc, char* argv[]){
    ...
    float *h_A;
    checkCudaErrors(cudaMallocManaged((void**)(&h_A), mem_size_A));
    init_matrix(h_A, size_A, 1.0);
    
    unsigned int size_B = dimsB.x * dimsB.y;
    unsigned int mem_size_B = sizeof(float) * size_B;
    float *h_B;
    checkCudaErrors(cudaMallocManaged((void**)(&h_B), mem_size_B));
    init_matrix(h_B, size_B, 2.0);

    unsigned int size_C = dimsC.x * dimsC.y;
    unsigned int mem_size_C = sizeof(float) * size_C;
    float *h_C;
    checkCudaErrors(cudaMallocManaged((void**)(&h_C), mem_size_C));
    ...
    // calculate time = HtoD + kernel + DtoH
    int niter = 300;
    ...
    for(int j = 0; j < niter; j++){
        ...
        for(int i = 0; i < GPU_N; i++){
            ...
            cudaMemPrefetchAsync(plan[i].h_B, mem_size_B, i, plan[i].stream);
            cudaMemPrefetchAsync(plan[i].h_pA, mem_size_A / GPU_N, i, plan[i].stream);
            cudaMemPrefetchAsync(plan[i].h_pC, mem_size_C / GPU_N, i, plan[i].stream);
            
            matrixMultiply<block_size>  <<< grid, threads, 0, plan[i].stream >>>(plan[i].h_pA, plan[i].h_B, plan[i].h_pC, dimsA.x, dimsB.x);
